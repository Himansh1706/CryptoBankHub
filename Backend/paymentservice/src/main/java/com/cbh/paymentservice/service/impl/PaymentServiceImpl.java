...
public class PaymentServiceImpl implements PaymentService {
	...
	private void checkQuantityAndThrowExceptionIfNotEnough(CryptoTransactionRequestDto cryptoTransactionRequestDto) {
		Boolean isEnoughQuantity = webClientConfig.get(String.format(
				"http://CRYPTO-SERVICE/check-quantity/%s/%s/%s", cryptoTransactionRequestDto.getUserId(),
				cryptoTransactionRequestDto.getCryptoId(), cryptoTransactionRequestDto.getQuantity()), Boolean.class);
	...
	private UserResponseDto getUserInfo(String userId) {
		return webClientConfig.get(String.format("http://USER-SERVICE/%s", userId), UserResponseDto.class);
	}
